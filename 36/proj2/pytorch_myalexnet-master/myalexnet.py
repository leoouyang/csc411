...
input = np.zeros((2,3,227,227))
input[0] = im
im = imread('dog2.png')[:,:,:3]
im = im - np.mean(im.flatten())
im = im/np.max(np.abs(im.flatten()))
...
im_v = Variable(torch.from_numpy(input), requires_grad=False)
softmax = torch.nn.Softmax()

print(softmax(model.forward(im_v)).data.numpy())
...
all_probs = softmax(model.forward(im_v)).data.numpy()[1]
sorted_ans = np.argsort(all_probs)

for i in range(-1, -6, -1):
    print("Answer2:", class_names[sorted_ans[i]], ", Prob:", all_probs[sorted_ans[i]])
